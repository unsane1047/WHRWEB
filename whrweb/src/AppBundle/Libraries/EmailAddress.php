...
// src/AppBundle/Libraries/EmailAddress.php
...
#
##################################################################################
/* code made into a class by Reid Johnson 10/10/2016 */

class EmailAddress{
	protected $address;

	function __construct( $string ){
		$$this->address = ( new UnicodeString( $string ) )->encode();
	}
	...
	function __invoke( $string ){
		$tmp = new EmailAddress( $string );
		return $tmp->__toString();
	}

	function get( $name ){
		if( $name === 'isValid' || $name === 'isvalid' || $name === 'is_valid' || $name === 'valid' )
			return $this->isValid();
	...
	function isValid(){
		$isValid = true;
		$atIndex = mb_strrpos( $this->address, "@" );
		if( $atIndex === false )
			$isValid = false;
		else
			$isValid = self::is_valid_email_address( $this->address, array( 'public_internet'	=> true ) );
	...
	protected static function email_strip_comments( $comment, $email, $replace='' ){
		while( 1 ){
