...
# 
namespace AppBundle\Libraries\fuzzymatchstrategies;
...
#  methods
	static function compute( $string ){
		$t = new self();
		$t->encode( $string );
		return array( $t->primary, $t->secondary );
	}

	function __invoke( $string ){
		$this->encode( $string );
		return array( $this->primary, $this->secondary );
	}

  protected function StringAt( $string, $start, $length, $list ){
    if( ( $start < 0 ) || ( $start >= mb_strlen( $string ) ) )
      return false;

    for( $i = 0; $i < count( $list ); $i++ ){
      if( $list[ $i ] === mb_substr( $string, $start, $length ) )
        return true;
    }
  ...
  protected function IsVowel( $string, $pos ){
    return preg_match( '/[AEIOUY]/', mb_substr( $string, $pos, 1 ) );
  }
  ...
  protected function encode( $string ){
    ...
    $this->original = mb_strtoupper( $this->original );
	$this->primary	= '';
	$this->secondary = '';
    ...
    if( mb_substr( $this->original, 0, 1) === 'X' ){
	...
	while( mb_strlen( $this->primary ) < 4 || mb_strlen( $this->secondary ) < 4 ){
		if( $this->current >= $this->length )
			break;

		switch( mb_substr( $this->original, $this->current, 1 ) ){
		case 'A':
		case 'E':
		case 'I':
		case 'O':
		...
		case 'B':
		  ...
		  if( mb_substr( $this->original, $this->current + 1, 1 ) === 'B' )
			$this->current += 2;
		  else
			$this->current++;
		...
		case 'C':
		  ...
		  if( ( $this->current > 1 ) 
			  ...
			  && $this->StringAt( $this->original, $this->current - 1, 3, 
						array('ACH') )
			  && (( mb_substr($this->original, $this->current + 2, 1) !== 'I')
				  && ((mb_substr($this->original, $this->current + 2, 1) !== 'E')
		  ...
		  if ($this->StringAt($this->original, $this->current, 2, array('CC'))
			...
			if ($this->StringAt($this->original, $this->current + 2, 1,
			  ...
			  if ((($this->current == 1)
				   && (mb_substr($this->original, $this->current - 1, 1) === 'A'))
				  || $this->StringAt($this->original, $this->current - 1, 5,
							array('UCCEE', 'UCCES'))) {
				$this->primary   .= 'KS';
				$this->secondary .= 'KS';
		...
		case 'F':
		  if (mb_substr($this->original, $this->current + 1, 1) === 'F')
			$this->current += 2;
		  else
			$this->current += 1;
		  $this->primary   .= 'F';
		...
		case 'G':
		  if (mb_substr($this->original, $this->current + 1, 1) === 'H') {
			if (($this->current > 0) 
			...
			if ($this->current < 3) {
			  # 'ghislane', 'ghiradelli'
			  if ($this->current == 0) {
				if (mb_substr($this->original, $this->current + 2, 1) === 'I') {
				  $this->primary   .= 'J';
				  $this->secondary .= 'J';
				} else {
				  $this->primary   .= 'K';
			...
			if ((($this->current > 1)
			  ...
			  if (($this->current > 2)
				  && (mb_substr($this->original, $this->current - 1, 1) === 'U')
				  && $this->StringAt($this->original, $this->current - 3, 1,
							array('C','G','L','R','T'))) {
				$this->primary   .= 'F';
				$this->secondary .= 'F';
			  } elseif (($this->current > 0)
						&& mb_substr($this->original, $this->current - 1, 1) !== 'I') {
				$this->primary   .= 'K';
				$this->secondary .= 'K';
			  }
			  $this->current += 2;
		  ...
		  if (mb_substr($this->original, $this->current + 1, 1) === 'N') {
			if (($this->current == 1) && $this->IsVowel($this->original, 0)
			  ...
			  if (!$this->StringAt($this->original, $this->current + 2, 2,
						  array('EY'))
				  && (mb_substr($this->original, $this->current + 1) !== 'Y')
				  && !$this->SlavoGermanic($this->original)) {
				 $this->primary   .= 'N';
				 $this->secondary .= 'KN';
			  } else {
		  ...
		  if (($this->current == 0)
			  && ((mb_substr($this->original, $this->current + 1, 1) === 'Y')
				  || $this->StringAt($this->original, $this->current + 1, 2,
		  ...
		  if (mb_substr($this->original, $this->current +1, 1) === 'G')
			$this->current += 2;
		  else
			$this->current += 1;
		...
		case 'J':
		  ...
		  if ($this->StringAt($this->original, $this->current, 4,
			...
			if ((($this->current == 0)
				 && (mb_substr($this->original, $this->current + 4, 1) === ' '))
				|| $this->StringAt($this->original, 0, 4, array('SAN '))) {
			  $this->primary   .= 'H';
			  $this->secondary .= 'H';
			} else {
		  ...
		  if (($this->current == 0)
			...
			if ($this->IsVowel($this->original, $this->current - 1)
				&& !$this->SlavoGermanic($this->original)
				&& ((mb_substr($this->original, $this->current + 1, 1) === 'A')
					|| (mb_substr($this->original, $this->current + 1, 1) === 'O'))) {
			  $this->primary   .= 'J';
			  $this->secondary .= 'H';
			} else {
		  ...
		  if (mb_substr($this->original, $this->current + 1, 1) === 'J') # it could happen
			$this->current += 2;
		  else 
			$this->current += 1;
		  break;
		...
		case 'K':
		  if (mb_substr($this->original, $this->current + 1, 1) === 'K')
			$this->current += 2;
		  else
			$this->current += 1;
		  $this->primary   .= 'K';
		...
		case 'L':
		  if (mb_substr($this->original, $this->current + 1, 1) === 'L') {
		  ...
		  if (($this->StringAt($this->original, $this->current - 1, 3,
			   ...
			   && ((($this->current + 1) == $this->last)
				   || $this->StringAt($this->original, $this->current + 2, 2,
							array('ER'))))
			  # 'dumb', 'thumb'
			  || (mb_substr($this->original, $this->current + 1, 1) === 'M')) {
			  $this->current += 2;
		  } else {
			  $this->current += 1;
		  }
		...
		case 'Q':
		  if (mb_substr($this->original, $this->current + 1, 1) === 'Q') 
			$this->current += 2;
		  else 
			$this->current += 1;
		  $this->primary   .= 'K';
		...
		case 'R':
		  ...
		  if (mb_substr($this->original, $this->current + 1, 1) === 'R') 
			$this->current += 2;
		  else
			$this->current += 1;
		  break;
		...
		case 'S':
		  ...
		  if ($this->StringAt($this->original, $this->current, 2,
					 array('SC'))) {
			# Schlesinger's rule 
			if (mb_substr($this->original, $this->current + 2, 1) === 'H')
			  ...
			  if ($this->StringAt($this->original, $this->current + 3, 2,
				...
				if (($this->current == 0) 
					...
					&& (mb_substr($this->original, $this->current + 3, 1) !== 'W')) {
				  $this->primary   .= 'X';
				  $this->secondary .= 'S';
				} else {
				  $this->primary   .= 'X';
		...
		case 'V':
		  if (mb_substr($this->original, $this->current + 1, 1) === 'V')
			$this->current += 2;
		  else
			$this->current += 1;
		  $this->primary   .= 'F';
		...
		case 'Z':
		  ...
		  if (mb_substr($this->original, $this->current + 1, 1) === 'H') {
			...
			break;
					...
					|| ($this->SlavoGermanic($this->original)
						&& (($this->current > 0)
							&& mb_substr($this->original, $this->current - 1, 1) !== 'T'))) {
			$this->primary   .= 'S';
			$this->secondary .= 'TS';
		  } else {
			$this->primary   .= 'S';
		  ...
		  if (mb_substr($this->original, $this->current + 1, 1) === 'Z')
			$this->current += 2;
		  else
			$this->current += 1;
		  break;
    ...
    $this->primary   = mb_substr( $this->primary,   0, 4 );
    $this->secondary = mb_substr( $this->secondary, 0, 4 );
